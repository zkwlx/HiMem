...
namespace {
    ...
    string_t get_dexfile_string_by_idx(uintptr_t dexfile, uintptr_t idx) {
        idx = idx;
    ...
    auto get_frame_size(
        ...
        if (is_runtime_method(method)) {
            if (frameptr == Read8(AccessArrayItem(callee_save_methods, kRefsAndArgs, 8UL))) {
                method_info = AccessArrayItem(callee_save_infos, kRefsAndArgs, 12UL);
            } else {
                if (frameptr == Read8(AccessArrayItem(callee_save_methods, kSaveAll, 8UL))) {
